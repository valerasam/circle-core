var Core = new (function () {
  ...
  function renderBackground() {
    ...
    gradient.addColorStop(0, "rgba(89, 89, 0, 1)");
  ...
  function documentMouseMoveHandler(event) {
    mouseX =
      event.clientX - (window.innerWidth - world.width) * 0.5 - BORDER_WIDTH;
    mouseY =
      event.clientY - (window.innerHeight - world.height) * 0.5 - BORDER_WIDTH;
  }
  ...
  function documentTouchStartHandler(event) {
    if (event.touches.length == 1) {
      ...
      mouseY =
        event.touches[0].pageY - (window.innerHeight - world.height) * 0.5;
  ...
  function documentTouchMoveHandler(event) {
    if (event.touches.length == 1) {
      ...
      mouseX =
        event.touches[0].pageX - (window.innerWidth - world.width) * 0.5 - 60;
      mouseY =
        event.touches[0].pageY - (window.innerHeight - world.height) * 0.5 - 30;
    }
  }
  ...
  function animate() {
    ...
    if (frameTime > timeLastSecond + 1000) {
      ...
      fps = Math.min(
        Math.round((frames * 1000) / (frameTime - timeLastSecond)),
        FRAMERATE
      );
    ...
    var scoreFactor =
      0.01 + (Math.max(Math.min(fps, FRAMERATE), 0) / FRAMERATE) * 0.99;
    ...
    if (playing) {
      ...
      // Core
      context.beginPath();
      context.fillStyle = "#9d9724";
      context.strokeStyle = "#dce23b";
    ...
    if (spaceIsDown && player.energy > 10) {
      ...
      context.beginPath();
      context.strokeStyle = "#8f9364";
      context.lineWidth = 1;
      context.fillStyle =
        "rgba(89, 89, 0, " + (player.energy / 100) * 0.9 + " )";
      context.arc(
        player.position.x,
        player.position.y,
        player.radius,
    ...
    for (i = 0; i < organisms.length; i++) {
      ...
      if (p.type == ORGANISM_ENEMY)
        context.fillStyle = "rgba( 131, 100, 147, " + p.alpha + " )";
      if (p.type == ORGANISM_ENERGY)
        context.fillStyle = "rgba( 231, 235, 0, " + p.alpha + " )";
      ...
      if (playing) {
        ...
        if (dist < 1.6) {
          if (
            p.distanceTo(player.position) > player.radius - 5 &&
            p.distanceTo(player.position) < player.radius + 5
          ) {
            p.dead = true;
    ...
    if (playing) {
      ...
      scoreText +=
        ...
        "s</span>";
      scoreText +=
        ' <p class="fps">FPS: <span>' +
        Math.round(fps) +
        " (" +
        Math.round(Math.max(Math.min(fps / FRAMERATE, FRAMERATE), 0) * 100) +
        "%)</span></p>";
      status.innerHTML = scoreText;
